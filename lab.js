var movies = [
 ...
 for (var movie of movies) {
    var upperCaseMovies = movie.title.toUpperCase();
    movie.title = upperCaseMovies;
    console.log(upperCaseMovies);
}

for ( var movie of movies) {
    if ( movie.title.toLowerCase() == 'citizen kane') {
        console.log('year of release: ', movie.year);
    } 
}

for (var i = 0; i < movies.length; i++) {
    var output = `title: ${movies[i].title}, audience rating: ${movies[i].ratings.audience}`
    console.log(output);
}
